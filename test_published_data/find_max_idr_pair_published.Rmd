...
head(countMatDiff)
 ...
 # getting the results name and addding to the coef we want to shrink
  ...
  volcano_plot_removed_reps = EnhancedVolcano(resLFC,
                  ...
                  y = 'padj', pCutoff = 0.05, labSize = 0 # if I make lab size 0 instead of 3 I can remove the rownames from being plotted over the points
...
# I shouldnt have this issue not that i used import on the peak files
...
# now I want to load the diff peaks in genes files and make a histogram to see their counts

```{r}

diff_peaks_in_up_genes = read_tsv("../intersect_peaks_in_genes/diff_peaks_in_up_genes_20kb.bed", col_names = FALSE)
head(diff_peaks_in_up_genes)

diff_peaks_in_down_genes = read_tsv("../intersect_peaks_in_genes/diff_peaks_in_down_genes_20kb.bed", col_names = FALSE)


diff_peaks_in_nochange_genes = read_tsv("../intersect_peaks_in_genes/diff_peaks_in_nochange_genes_20kb.bed", col_names = FALSE)


df_diff_peaks_up_genes = DataFrame(diff_peaks_up_genes = diff_peaks_in_up_genes$X7)
up_peaks_in_up_genes_num = length(which("up_regulated_peaks.bed" == df_diff_peaks_up_genes$diff_peaks_up_genes))
down_peaks_in_up_genes_num = length(which("down_regulated_peaks.bed" == df_diff_peaks_up_genes$diff_peaks_up_genes))



head(df_diff_peaks_up_genes, 20) 
 
df_diff_peaks_down_genes = DataFrame(diff_peaks_down_genes = diff_peaks_in_down_genes$X7)
up_peaks_in_down_genes_num = length(which("up_regulated_peaks.bed" == df_diff_peaks_down_genes$diff_peaks_down_genes))
down_peaks_in_down_genes_num = length(which("down_regulated_peaks.bed" == df_diff_peaks_down_genes$diff_peaks_down_genes))

df_diff_peaks_nochange_genes = DataFrame(diff_peaks_nochange_genes = diff_peaks_in_nochange_genes$X7)
up_peaks_in_nochange_genes_num = length(which("up_regulated_peaks.bed" == df_diff_peaks_nochange_genes$diff_peaks_nochange_genes))
down_peaks_in_nochange_genes_num = length(which("down_regulated_peaks.bed" == df_diff_peaks_nochange_genes$diff_peaks_nochange_genes))


peaks_in_genes_df = data.frame(up_peaks_in_up_genes_num, down_peaks_in_up_genes_num, up_peaks_in_down_genes_num, down_peaks_in_down_genes_num, up_peaks_in_nochange_genes_num, down_peaks_in_nochange_genes_num)

peaks_in_genes_df

long_peaks_in_genes_df = pivot_longer(peaks_in_genes_df, cols = everything(), names_to = "peaks_genes_category", values_to = "counts")



diff_peaks_in_genes_plot = ggplot(long_peaks_in_genes_df, aes(peaks_genes_category, counts))+
  geom_count()+
  theme(axis.text.x = element_text(angle = 75 , hjust = 1))+
  scale_y_continuous(labels = scales::label_number())
ggsave("../intersect_peaks_in_genes/counts_of_diff_peaks_in_diff_genes.png", diff_peaks_in_genes_plot)


# to view it here
ggplot(long_peaks_in_genes_df, aes(peaks_genes_category, counts))+
  geom_count()+
  theme(axis.text.x = element_text(angle = 75, hjust = 1))+
  scale_y_continuous(labels = scales::label_number())
#ggplot(df_diff_peaks_up_genes, aes("diff_peaks_up_genes"))+
