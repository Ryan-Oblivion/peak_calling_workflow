...
```
--knownGene_bed_file 'path to one knowngene bed file' : use this parameter if you want to change the default knowngene file from hg38 to a different organism or version

--proseq_up_genes 'path to one up genes bed file' : use this if you have pro-seq data and a bed file with the up regulated genes from an experiment of the same type as your peak calling experiment used. The up genes should come from the experiment design where treatment vs control(wt)
--proseq_down_genes : similar to the proseq_up_genes parameter, but these would be the down proseq genes
--proseq_unchnaging_genes : similar to the up and down proseq genes but the unchanging genes


Use these parameters if you have pro-seq bam files and want to get a bar plot showing the log2fc over roadmap epigenetic states
Must be a pair glob pattern, with example
...
Use these parameters to pass in roadmap epigenetic state bed files which can be a glob pattern that points to all of the type of peaks in the directory
...
```
This section should not be confused with the atac_analysis path below
This is for when you are running atac-seq or narrow histone mark data through the pipeline as the main data you're analyzing

Use these parameters when you have histone bam files and want to have a bar graph showing log2fc of this signal over any of your atac peaks that are proximal or distal from gene tss.

But this workflow was not fully complete so these two parameters are not needed right now
...
```
--atac_analysis true or false (default false) : I mentioned this a few sections up, but for running atac analysis just set this and it will be true or you can write true next to it in the command line. Next, specify the locations of your atac-seq bam files and atac seq peak bed files so you can get heat maps and profile plots where signal from the current assay (example cut&tag ) will have  cut&tag signal plotted over peaks or atac-seq signal plotted over cut&tag signal
The atac analysis will also calculate the log2FC of the atac-seq signal over different epigenetic states

Use these two parameters to get your ATAC-seq bigwig into the pipeline to get heatmaps that involve atac-seq data
You might need to run this pipeline first through the atac seq path to get the atac seq bigwigs and peak files
Then run the pipeline again in a different directory where you specify that path that points to the bigwig outputs when using this
...
If you ran the atac seq path of this pipeline or have your own atac seq up, down, and unchanging peaks, then you can have a path that points to them here so you can get heatmaps of the cut&tag (or any peak based assay) signal over these peak
